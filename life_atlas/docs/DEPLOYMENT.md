...
2. Add a concise, user-facing entry for the release to `CHANGELOG.md`. Home Assistant displays this file in its update dialog, and validation rejects a current version with no matching entry.
3. Run `python scripts/deploy.py`. To bump the release version at the same time, run `python scripts/deploy.py --set-version <version>`.
4. A healthy preflight prints one line beginning `LIFE_ATLAS_DEPLOY=` with `version_sync:true`, `validation:"pass"`, and `release_ready:true`. Use `--verbose` only if that compact result reports a failure.
5. Merge/push the focused change to private `main`. The publish workflow emits `LIFE_ATLAS_PUBLISH={...}` and synchronizes the allowlisted application snapshot, including `CHANGELOG.md`, to the public distribution repository.
6. The public image workflow emits `LIFE_ATLAS_IMAGE={...}`. Check job status or that compact summary first; fetch verbose logs only for a failed job.
7. In Home Assistant, confirm the update dialog shows the expected changelog, create a backup, and install the offered Life Atlas app update.
8. Confirm the app is started and request `/api/health` through Home Assistant Ingress. The deployment is complete when health is `ok` and the reported version matches the release. Check recent app logs only if this verification fails.
...
Life Atlas keeps the ten most recent **Back up** archives under `/data/backups` and the ten most recent automatic rollback snapshots under `/data/restore-backups`, deleting older ones as new ones are written. `/data` is Supervisor-managed and finite, so neither store may grow without bound. Change the limits with `LIFE_ATLAS_MAX_BACKUPS` and `LIFE_ATLAS_MAX_ROLLBACK_SNAPSHOTS`; the snapshot written by a restore is never pruned by that same restore. Home Assistant's own backups are unaffected.
...
2. Add the release notes to `CHANGELOG.md`, then bump `config.yaml` and the `LIFE_ATLAS_VERSION` value in `run.sh` together. `scripts/deploy.py --set-version <version>` performs the version edit and validates that the changelog contains the new version.
