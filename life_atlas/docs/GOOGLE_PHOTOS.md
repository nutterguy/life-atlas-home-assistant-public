...
The Picker workflow is for deliberate attachment of up to 50 selected photos to an event, diary day, or person. It cannot silently scan the whole Google Photos library. Google shares only items explicitly selected in its Picker UI.
...
9. Open an event, diary day, or person and choose **Choose photos from Google Photos**.
...
Choose **Disconnect and forget Picker setup** to revoke the browser token when possible and remove the saved Picker Web client ID. This does not delete photos already copied into Life Atlas. Use **Remove** beside a photo to remove the Life Atlas association and its local copy when no other Life Atlas record uses that file. The original Google Photos item is never deleted.
