...
- `connectors.py`, `connector_http.py`, `mock_connector.py`: the source-neutral connector boundary and its HTTP transport, described in `docs/CONNECTORS.md`.
- `connector_registry.py`: the plug-in registry. Each connector is registered with an identity, a direction, an address, Life Atlas's own key to it and an on/off switch, stored in `/data/connectors.sqlite3` so that no machine address or credential enters the canonical database. It serves `/api/connectors` and backs the Sources view in `static/connector-tools.js`.
...
The WhatsApp durable archive belongs to the separate Life Atlas WhatsApp Archive app and lives in that app's own `/data`. It and Life Atlas communicate only through Connector Protocol v1; neither opens the other's SQLite database. Backing up the archive and its pairing state is that app's own concern, and a Life Atlas backup does not contain them.
