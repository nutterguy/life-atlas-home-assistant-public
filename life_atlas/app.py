...
def initialise():
    ...
    with closing(connect()) as con, con:
        ...
        if seed_sample and con.execute("SELECT COUNT(*) FROM chapters").fetchone()[0] == 0:
            con.execute("INSERT INTO chapters(title,start_date,end_date,summary,color,confidence) VALUES(?,?,?,?,?,?)", ("A year of change", "2025-01-01", "2025-12-31", "Travel, a new role, festivals and outdoor events.", "#8ba3ff", .7))
...
def seed(con):
    places = [
        ("Valletta", "Valletta", "", "Malta", 35.8997, 14.5147),
        ("nDreams", "Farnborough", "Hampshire", "United Kingdom", 51.2869, -0.7526),
        ("Hurghada", "Hurghada", "Red Sea", "Egypt", 27.2579, 33.8116),
        ("Glastonbury Festival", "Pilton", "Somerset", "United Kingdom", 51.1536, -2.5852),
        ("Ubud", "Ubud", "Bali", "Indonesia", -8.5069, 115.2625),
    ]
    con.executemany("INSERT INTO places(name,city,region,country,latitude,longitude) VALUES(?,?,?,?,?,?)", places)
    sources = [("Gmail", "email"), ("Google Calendar", "calendar"), ("App in the Air", "travel_export"), ("WhatsApp", "chat")]
    con.executemany("INSERT INTO sources(name,source_type) VALUES(?,?)", sources)
    con.execute("INSERT INTO people(name,relationship) VALUES('Lucy','Partner')")
    con.execute("INSERT INTO trips(title,start_date,end_date,status,notes) VALUES('Egypt holiday','2025-03-22','2025-03-28','confirmed','Sample record')")
    trip_id = con.execute("SELECT id FROM trips WHERE title='Egypt holiday'").fetchone()[0]
    event_rows = [
        ("Indonesia trip", "2016-09-02", "2016-09-18", "Travel", "confirmed", .86, "major", 5, None, "clear"),
        ("Egypt holiday with Lucy", "2025-03-22", "2025-03-28", "Travel", "confirmed", .95, "major", 3, trip_id, "clear"),
        ("Accepted Producer role at nDreams", "2025-05-19", "2025-05-19", "Career", "confirmed", 1, "major", 2, None, "clear"),
        ("Started at nDreams", "2025-06-10", "2025-06-10", "Career", "confirmed", 1, "major", 2, None, "clear"),
        ("Glastonbury Festival", "2025-06-25", "2025-06-29", "Music", "booked", .75, "major", 4, None, "needs_review"),
        ("Malta summer trip", "2025-07-11", "2025-07-16", "Travel", "uncertain", .55, "medium", 1, None, "needs_review"),
    ]
    con.executemany("""INSERT INTO events(title,start_date,end_date,category,status,confidence,importance,place_id,trip_id,review_state)
                       VALUES(?,?,?,?,?,?,?,?,?,?)""", event_rows)
    egypt = con.execute("SELECT id FROM events WHERE title='Egypt holiday with Lucy'").fetchone()[0]
    lucy = con.execute("SELECT id FROM people WHERE name='Lucy'").fetchone()[0]
    con.execute("INSERT INTO event_people(event_id,person_id,role) VALUES(?,?,'with')", (egypt, lucy))
    gmail = con.execute("SELECT id FROM sources WHERE name='Gmail'").fetchone()[0]
    con.execute("INSERT INTO evidence(event_id,source_id,evidence_type,source_ref,excerpt,confidence) VALUES(?,?,?,?,?,?)", (egypt, gmail, "booking", "sample-email-ref", "Flight and accommodation confirmations", .95))
    for title in ("Glastonbury Festival", "Malta summer trip"):
        event = con.execute("SELECT id, confidence FROM events WHERE title=?", (title,)).fetchone()
        con.execute("INSERT INTO review_items(event_id,issue_type,summary,details) VALUES(?,?,?,?)", (event["id"], "attendance", f"Confirm attendance: {title}", "Booked or mentioned, but confirmed attendance evidence is incomplete."))
