...
# /data is Supervisor-managed and finite; every Back up click wrote a new archive
# containing the database, imports and all media, and nothing ever removed one.
...
def migrate(con):
    ...
    if current_version > SCHEMA_VERSION:
        raise ValueError(f"Database schema version {current_version} is newer than this Life Atlas build")
...
def sample_seed_data():
    """Synthetic demonstration records shipped alongside the app.

    Kept in sample-seed.json rather than inline so the sample data is obviously
    data, reviewable on its own, and checkable by scripts/validate_repository.py.
    """
    return json.loads((RESOURCE_ROOT / "sample-seed.json").read_text(encoding="utf-8"))


def seed(con):
    data = sample_seed_data()
    con.executemany(
        "INSERT INTO places(name,city,region,country,latitude,longitude) VALUES(?,?,?,?,?,?)",
        [(p["name"], p["city"], p["region"], p["country"], p["latitude"], p["longitude"])
         for p in data["places"]])
    con.executemany(
        "INSERT INTO sources(name,source_type) VALUES(?,?)",
        [(s["name"], s["source_type"]) for s in data["sources"]])
    con.executemany(
        "INSERT INTO people(name,relationship) VALUES(?,?)",
        [(p["name"], p["relationship"]) for p in data["people"]])
    con.executemany(
        "INSERT INTO trips(title,start_date,end_date,status,notes) VALUES(?,?,?,?,?)",
        [(t["title"], t["start_date"], t["end_date"], t["status"], t["notes"]) for t in data["trips"]])
    places = {row["name"]: row["id"] for row in con.execute("SELECT id, name FROM places")}
    trips = {row["title"]: row["id"] for row in con.execute("SELECT id, title FROM trips")}
    con.executemany(
        """INSERT INTO events(title,start_date,end_date,category,status,confidence,importance,place_id,trip_id,review_state)
                       VALUES(?,?,?,?,?,?,?,?,?,?)""",
        [(e["title"], e["start_date"], e["end_date"], e["category"], e["status"], e["confidence"],
          e["importance"], places.get(e["place"]), trips.get(e["trip"]), e["review_state"])
         for e in data["events"]])
    events = {row["title"]: row["id"] for row in con.execute("SELECT id, title FROM events")}
    people = {row["name"]: row["id"] for row in con.execute("SELECT id, name FROM people")}
    sources = {row["name"]: row["id"] for row in con.execute("SELECT id, name FROM sources")}
    con.executemany(
        "INSERT INTO event_people(event_id,person_id,role) VALUES(?,?,?)",
        [(events[link["event"]], people[link["person"]], link["role"]) for link in data["event_people"]])
    con.executemany(
        "INSERT INTO evidence(event_id,source_id,evidence_type,source_ref,excerpt,confidence) VALUES(?,?,?,?,?,?)",
        [(events[item["event"]], sources[item["source"]], item["evidence_type"], item["source_ref"],
          item["excerpt"], item["confidence"]) for item in data["evidence"]])
    con.executemany(
        "INSERT INTO review_items(event_id,issue_type,summary,details) VALUES(?,?,?,?)",
        [(events[item["event"]], item["issue_type"], item["summary"], item["details"])
         for item in data["review_items"]])


def seed_chapters(con):
    con.executemany(
        "INSERT INTO chapters(title,start_date,end_date,summary,color,confidence) VALUES(?,?,?,?,?,?)",
        [(c["title"], c["start_date"], c["end_date"], c["summary"], c["color"], c["confidence"])
         for c in sample_seed_data()["chapters"]])
...
def update_event(event_id, payload):
    ...
    with closing(connect()) as con, con:
        ...
        con.execute("""UPDATE events SET title=?,start_date=?,end_date=?,description=?,category=?,status=?,
          confidence=?,importance=?,place_id=?,trip_id=?,review_state=?,updated_at=CURRENT_TIMESTAMP WHERE id=?""",
...
def prune_backups(keep=None):
    """Keep only the newest archives so /data cannot grow without bound."""
    keep = MAX_BACKUP_ARCHIVES if keep is None else keep
    archives = sorted(BACKUPS.glob("life-atlas-backup-*.zip"), reverse=True)
    removed = []
