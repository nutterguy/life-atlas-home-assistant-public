...
DEFAULT_WHATSAPP_CONNECTOR_URL = "http://127.0.0.1:8097"
DEFAULT_WHATSAPP_KEY_FILE = "/data/whatsapp/secrets/connector-api-key"
...
def whatsapp_connector_url() -> str:
    return os.environ.get("LIFE_ATLAS_WHATSAPP_CONNECTOR_URL", DEFAULT_WHATSAPP_CONNECTOR_URL).rstrip("/")


def whatsapp_connector_key_file() -> Path:
    return Path(os.environ.get("LIFE_ATLAS_WHATSAPP_CONNECTOR_KEY_FILE", DEFAULT_WHATSAPP_KEY_FILE))
...
def make_whatsapp_client(
    *,
    ...
    key = (key_file or whatsapp_connector_key_file()).read_text(encoding="ascii").strip()
    if len(key) < 32:
        raise ConnectorError("WhatsApp connector key is unavailable")
    return ConnectorClient(
        WHATSAPP_CONNECTOR_ID,
        HTTPConnectorTransport(
            base_url or whatsapp_connector_url(),
            timeout=timeout,
            headers={"X-Life-Atlas-Connector-Key": key},
        ),
    )
...
def whatsapp_connector_diagnostic(
    *,
    query: str | None = None,
    base_url: str | None = None,
    key_file: Path | None = None,
    ...
    """Return bounded WhatsApp archive status without affecting core availability."""
    started = perf_counter()
    try:
        client = make_whatsapp_client(base_url=base_url, key_file=key_file, timeout=timeout)
        info = client.info()
        status = client.status()
        capabilities = client.capabilities()
        result: dict[str, Any] = {
