...
# list_backups() only ever showed ten; nothing deleted the rest.
MAX_ROLLBACK_SNAPSHOTS = max(1, int(os.environ.get("LIFE_ATLAS_MAX_ROLLBACK_SNAPSHOTS", "10")))
...
VERSION_TWO_COLUMNS = {
    "ingestion_runs": {"id", "connector", "package_checksum", "status"},
    "source_records": {"id", "source_id", "object_type", "external_id", "content_sha256"},
    "event_source_records": {"event_id", "source_record_id", "relationship"},
    "agent_mutations": {"id", "idempotency_key", "operation", "target_type", "target_id"},
}
...
class RestoreManager:
    ...
    def _switch_database(self, candidate: Path, *, source: str, source_checksum: str, staged_media: Path | None = None) -> dict:
        ...
        pruned = self._prune_rollback_snapshots(keep_id=backup_id)
        self._audit("committed", backup_id=backup_id, source=source, checksum=source_checksum, counts=report["counts"], media=media_result, pruned_snapshots=len(pruned))
    ...
    def _inspect_database(self, path: Path, *, check_media: bool, media_root: Path | None = None) -> dict:
        ...
        try:
            ...
            if user_version >= 2:
                required_columns.update(VERSION_TWO_COLUMNS)
    ...
    def _prune_rollback_snapshots(self, *, keep_id: str) -> list[str]:
        """Keep the newest snapshots, never removing the one just written."""
        snapshots = sorted(
            (path for path in self.backups.glob("*.sqlite3") if BACKUP_ID.fullmatch(path.stem)),
            reverse=True,
        )
        removed = []
        for stale in snapshots[MAX_ROLLBACK_SNAPSHOTS:]:
            if stale.stem == keep_id:
                continue
            try:
                stale.unlink()
