...
class RestoreServiceTests(unittest.TestCase):
    ...
    def test_version_one_restore_is_prepared_as_version_two(self):
        ...
        try:
            with con:
                ...
                con.execute("DROP TABLE agent_mutations")
                con.execute("PRAGMA user_version=1")
        finally:
            con.close()
        session = self.upload(candidate)
        validated = self.manager.validate_session(session["id"], session["token"])
        self.assertEqual(validated["report"]["user_version"], 2)
