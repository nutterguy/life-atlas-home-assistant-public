...
class GooglePhotosPickerTests(unittest.TestCase):
    ...
    def test_picker_imports_multiple_pages_for_events_and_people(self):
        with closing(self.connect()) as con, con:
            event_id = con.execute(
                "INSERT INTO events(title,start_date,end_date) VALUES('Test event','2026-08-21','2026-08-21')"
            ).lastrowid
            person_id = con.execute("INSERT INTO people(name) VALUES('Test person')").lastrowid

        created_sessions = []

        def response(url, **kwargs):
            if url.endswith("/sessions") and kwargs.get("method") == "POST":
                created_sessions.append(kwargs["body"])
                return {"id": f"google-session-{len(created_sessions)}",
                        "pickerUri": "https://photos.google.com/picker"}
            if "/sessions/google-session-" in url and kwargs.get("method", "GET") == "GET":
                return {"mediaItemsSet": True}
            if "/mediaItems?" in url:
                second_page = "pageToken=next" in url
                suffix = "b" if second_page else "a"
                page = {"mediaItems": [{"id": f"photo-{suffix}", "type": "PHOTO",
                                         "createTime": "2026-08-21T14:00:00Z",
                                         "mediaFile": {"baseUrl": f"https://lh3.googleusercontent.com/{suffix}",
        ...
        with patch.object(picker, "_request_json", side_effect=response), \
             patch.object(picker, "_download_photo", return_value=self.photo):
            event_session = picker.create_session(self.data, {"event_id": event_id}, access_token="x" * 30)
            event_result = picker.poll_session(self.data, self.connect, event_session["session_id"])
            self.assertEqual(event_result["count"], 2)
            person_session = picker.create_session(self.data, {"person_id": person_id}, access_token="x" * 30)
            person_result = picker.poll_session(self.data, self.connect, person_session["session_id"])
            self.assertEqual(person_result["count"], 2)

        self.assertEqual(created_sessions[0]["pickingConfig"]["maxItemCount"], "50")
        with closing(self.connect()) as con, con:
            self.assertEqual(con.execute("SELECT COUNT(*) FROM media WHERE event_id=?", (event_id,)).fetchone()[0], 2)
            self.assertEqual(con.execute("SELECT COUNT(*) FROM media WHERE person_id=?", (person_id,)).fetchone()[0], 2)
            self.assertEqual(con.execute("SELECT COUNT(DISTINCT source_ref) FROM media").fetchone()[0], 2)
    ...
    def test_windows_dpapi_round_trip(self):
        ...
        try:
            protected = picker._dpapi(secret, protect=True)
        except ValueError as exc:
            self.skipTest(f"DPAPI is unavailable for this Windows test account: {exc}")
        self.assertEqual(picker._dpapi(protected, protect=False), secret)
