...
class GooglePhotosMcpProxyTests(unittest.TestCase):
    ...
    def test_status_reports_service_available_before_oauth(self):
        with mock.patch.object(
            proxy,
            "request_local",
            return_value=(503, "Service Unavailable", [], b'{"status":"unhealthy"}'),
        ), mock.patch.object(
            proxy, "TOKEN_DB", Path("/definitely/not/present/tokens.db")
        ), mock.patch.dict(
            os.environ,
