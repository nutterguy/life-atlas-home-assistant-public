...
This private repository is the canonical source for the Home Assistant edition. Personal data is not committed here; it lives in the add-on's persistent `/data` directory. The private Windows edition repository holds the canonical portable database snapshot.
...
- Keep `schema.sql` compatible with the private Windows edition repository.
