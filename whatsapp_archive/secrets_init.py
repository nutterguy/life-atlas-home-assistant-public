...
def options() -> dict[str, object]:
    try:
        value = json.loads((DATA / "options.json").read_text(encoding="utf-8"))
...
def main() -> None:
    ...
    raw = options()
    session_name = str(raw.get("session_name", "life-atlas"))
    ...
    config = {
        ...
        "reconcile_seconds": integer(raw.get("reconcile_seconds"), 300, 30, 3600),
        "full_rescan_hours": integer(raw.get("full_rescan_hours"), 24, 1, 168),
        "history_page_size": integer(raw.get("history_page_size"), 500, 50, 1000),
    }
