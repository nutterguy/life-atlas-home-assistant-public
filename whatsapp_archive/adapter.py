...
from archive import Archive, utc_now
...
class Service:
    def __init__(self) -> None:
        ...
        self.pairing_file = DATA / "secrets" / "pairing.json"
    ...
    def pairing_record(self) -> dict[str, Any]:
        try:
            return json.loads(self.pairing_file.read_text(encoding="utf-8"))
        except (OSError, ValueError):
            return {}

    def claim_connector_key(self, peer: str) -> str:
        """Hand the connector key to the first caller, once.

        Life Atlas and this app are both installed by the same person on the
        same machine, so making them introduce themselves through a copied
        secret is ceremony, not security. The key is given to whoever asks
        first and to nobody after: the claim is recorded and shown on this
        page, so a claim you did not expect is visible rather than silent, and
        `forget_pairing` lets you hand it over again deliberately.
        """
        if self.pairing_record().get("claimed_at"):
            raise PermissionError("This connector has already been paired")
        key = read_secret(self.connector_key_file)
        if not key:
            raise RuntimeError("Connector key is not available yet")
        self.pairing_file.parent.mkdir(parents=True, exist_ok=True)
        self.pairing_file.write_text(
            json.dumps({"claimed_at": utc_now(), "peer": peer[:120]}),
            encoding="utf-8",
        )
        return key
    ...
    def chat_inventory(self, *, limit: int, offset: int) -> dict[str, Any]:
        ...
        return {
            ...
            "pairing": self.pairing_record() or {"claimed_at": None, "peer": None},
        }

    def chat_messages(self, chat_id: str, *, limit: int, offset: int) -> dict[str, Any]:
        return self.archive.chat_messages(chat_id, limit=limit, offset=offset)
...
class ConnectorHandler(HandlerBase):
    ...
    def do_POST(self) -> None:
        route = urlparse(self.path).path
        if route == "/v1/pair":
            return self.dispatch_json(self._pair)
        if not self._authorised():
            return
        if route == "/v1/search":
            ...
            self.dispatch_json(self._context)
        else:
            self.send_error_json(404, "not_found", "Unknown Connector Protocol endpoint")

    def _pair(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        peer = str(payload.get("peer") or self.client_address[0] or "unknown")
        try:
            key = SERVICE.claim_connector_key(peer)
        except PermissionError as exc:
            raise ValueError(str(exc)) from exc
        return {"connector_id": "whatsapp", "connector_key": key}
    ...
    def _context(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        source_id = payload.get("source_id")
        if not isinstance(source_id, str) or not source_id or len(source_id) > 512:
            raise ValueError("source_id is invalid")
        before = payload.get("before", 5)
        after = payload.get("after", 5)
        ...
        context = SERVICE.archive.context(source_id, before=before, after=after)
        if context is None:
            raise ValueError("source item was not found")
        return context
...
class ManagementHandler(HandlerBase):
    def do_GET(self) -> None:
        ...
        elif route == "/manage/chats/messages":
            query = parse_qs(urlparse(self.path).query)
            try:
                chat_id = query.get("chat_id", [""])[0]
                limit = int(query.get("limit", ["40"])[0])
                offset = int(query.get("offset", ["0"])[0])
                self.send_json(200, SERVICE.chat_messages(chat_id, limit=limit, offset=offset))
            except (TypeError, ValueError) as exc:
                self.send_error_json(400, "bad_request", str(exc))
        else:
    ...
    def do_POST(self) -> None:
        ...
        if self.headers.get("X-Life-Atlas-Action") != "1":
            ...
            return
        actions = {
            "/manage/pairing/forget": lambda _: (SERVICE.forget_pairing(), {"paired": False})[1],
...
<section class="card"><h2>Life Atlas connector key</h2><p class="muted">Life Atlas needs this key to read only the selected durable archive.</p><button class="secondary" onclick="key()">Reveal key</button><pre id="key" hidden></pre></section>
<section class="card"><h2>Diagnostics</h2><pre id="diagnostics">Loading…</pre></section>
</main><dialog id="preview"><div class="row"><div><strong id="preview-title">Chat preview</strong><div id="preview-id" class="muted"></div></div><button class="secondary" onclick="document.getElementById('preview').close()">Close</button></div><p class="muted">Read-only messages from the durable, backed-up archive.</p><div id="messages" class="messages"></div><button id="older" class="secondary" onclick="loadOlder()">Load older messages</button></dialog><script>
...
async function loadChats(){let all=[],offset=0,selection=null;for(let page=0;page<20;page++){const v=await jsonFetch(`manage/chats?limit=500&offset=${offset}`);all=all.concat(v.chats);selection=v.selection;if(v.next_offset===null)break;offset=v.next_offset}const body=document.getElementById('chats');body.replaceChildren();if(!all.length){const tr=document.createElement('tr'),td=document.createElement('td');td.colSpan=7;td.textContent='No chats discovered yet.';tr.append(td);body.append(tr);return}for(const chat of all){const tr=document.createElement('tr');const title=document.createElement('td');const strong=document.createElement('strong');strong.textContent=chat.name||chat.chat_id;const small=document.createElement('small');small.textContent=chat.name?chat.chat_id:chat.chat_type;title.append(strong,document.createElement('br'),small);const state=document.createElement('td');state.textContent=chat.archived?'Archived':chat.chat_type;const count=document.createElement('td');count.textContent=String(chat.observed_message_count);const range=document.createElement('td');range.textContent=`${chat.observed_earliest||'—'} → ${chat.observed_latest||'—'}`;const policy=document.createElement('td');const select=document.createElement('select');for(const [value,label] of [['auto','Automatic'],['include','Always include'],['exclude','Always exclude']]){const option=document.createElement('option');option.value=value;option.textContent=label;option.selected=chat.mode===value;select.append(option)}select.onchange=()=>setPolicy(chat.chat_id,select.value);policy.append(select);const durable=document.createElement('td');durable.className=chat.effective_selected?'included':'excluded';durable.textContent=chat.effective_selected?`Include · ${chat.durable_message_count} stored`:'Exclude';const preview=document.createElement('td'),previewButton=document.createElement('button');previewButton.className='secondary';previewButton.textContent='Messages';previewButton.disabled=!chat.effective_selected||!chat.durable_message_count;previewButton.onclick=()=>openPreview(chat);preview.append(previewButton);tr.append(title,state,count,range,policy,durable,preview);body.append(tr)}}
let previewChat=null,previewOffset=0;
function appendMessages(items,prepend=false){const box=document.getElementById('messages'),nodes=items.map(item=>{const message=document.createElement('div');message.className='message'+(item.metadata.from_me?' mine':'');const body=document.createElement('div');body.textContent=item.text||`[${item.metadata.message_type||'message'}]`;const meta=document.createElement('small');meta.textContent=`${item.metadata.from_me?'You':(item.participants[0]?.label||'Contact')} · ${item.timestamp||'Unknown time'}`;message.append(body,meta);return message});if(prepend)box.prepend(...nodes);else box.append(...nodes)}
async function openPreview(chat){previewChat=chat;previewOffset=0;document.getElementById('preview-title').textContent=chat.name||chat.chat_id;document.getElementById('preview-id').textContent=`${chat.chat_id} · ${chat.durable_message_count} stored`;document.getElementById('messages').replaceChildren();document.getElementById('preview').showModal();await loadOlder()}
async function loadOlder(){if(!previewChat)return;try{const page=await jsonFetch(`manage/chats/messages?chat_id=${encodeURIComponent(previewChat.chat_id)}&limit=40&offset=${previewOffset}`);appendMessages(page.items,previewOffset>0);previewOffset=page.next_offset;document.getElementById('older').hidden=previewOffset===null}catch(e){alert(e.message)}}
...
async function key(){try{const v=await jsonFetch('manage/connector-key');const el=document.getElementById('key');el.textContent=v.connector_api_key;el.hidden=false}catch(e){alert(e.message)}}
...
def main() -> None:
    ...
    SERVICE = Service()
    SERVICE.start()
    connector = ThreadingHTTPServer(("0.0.0.0", CONNECTOR_PORT), ConnectorHandler)
    management = ThreadingHTTPServer(("0.0.0.0", MANAGEMENT_PORT), ManagementHandler)
