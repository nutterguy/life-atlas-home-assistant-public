...
class FakeService:
    ...
    def chat_messages(self, chat_id, *, limit, offset):
...
class AdapterTests(unittest.TestCase):
    ...
    def test_connector_search_changes_and_item(self):
        ...
        _, context = self.json_request(
            self.connector_url + "/v1/context",
            body={"source_id": event["id"], "before": 5, "after": 5},
            headers=headers,
            method="POST",
        )
        self.assertEqual(context["focus_source_id"], event["id"])
        self.assertEqual(context["items"][0]["text"], "Dinner booked at seven")

    def test_management_chat_preview_is_read_only_and_selection_gated(self):
        chat_id = "preview@c.us"
        self.service.archive.upsert_chat({"id": chat_id, "name": "Preview person", "archived": False})
        self.service.archive.replace_inventory_metrics({chat_id: (2, 1_700_000_010, 1_700_000_020)})
        self.service.archive.confirm_selection()
        for suffix, timestamp, body in (("1", 1_700_000_010, "Earlier"), ("2", 1_700_000_020, "Later")):
            self.service.archive.upsert_message({"id": f"false_{chat_id}_{suffix}", "from": chat_id,
                "chatId": chat_id, "timestamp": timestamp, "body": body, "type": "chat"})
        _, page = self.json_request(self.management_url +
            "/manage/chats/messages?chat_id=preview%40c.us&limit=1&offset=0")
        self.assertEqual(page["items"][0]["text"], "Later")
        self.assertEqual(page["next_offset"], 1)
        self.service.archive.set_chat_policy(chat_id, "exclude")
        with self.assertRaises(HTTPError) as caught:
            urlopen(self.management_url + "/manage/chats/messages?chat_id=preview%40c.us", timeout=3)
