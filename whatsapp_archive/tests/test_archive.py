...
class ArchiveTests(unittest.TestCase):
    ...
    def test_direct_chat_name_is_recovered_from_incoming_sender(self):
        chat_id = "12345@lid"
        self.archive.upsert_chat({"id": chat_id, "archived": False})
        self.archive.replace_inventory_metrics({chat_id: (1, 1_700_000_000, 1_700_000_000)})
        self.archive.confirm_selection()
        payload = {
            **message(),
        ...
        item = self.archive.get_item("direct-name-1")
        self.assertEqual(item["metadata"]["conversation_name"], "Alex Example")
        self.assertEqual(self.archive.get_chat_policy(chat_id)["name"], "Alex Example")

    def test_context_returns_only_nearby_messages_from_the_same_chat(self):
        chat_id = "context@c.us"
        other_id = "other@c.us"
        self.approve_chat(chat_id)
        self.archive.upsert_chat({"id": other_id, "name": "Other", "archived": False})
        for index in range(5):
            self.archive.upsert_message({
                **message(), "id": f"context-{index}", "chatId": chat_id,
                "from": chat_id, "timestamp": 1_700_000_000 + index,
                "body": f"Context {index}",
            })
        self.archive.upsert_message({
            **message(), "id": "other-1", "chatId": other_id,
            "from": other_id, "timestamp": 1_700_000_002, "body": "Other chat",
        })
        context = self.archive.context("context-2", before=1, after=2)
        self.assertEqual(context["focus_source_id"], "context-2")
        self.assertEqual(
            [item["source_id"] for item in context["items"]],
            ["context-1", "context-2", "context-3", "context-4"],
        )
        self.assertNotIn("other-1", [item["source_id"] for item in context["items"]])
    ...
    def test_inventory_prefers_sender_label_over_numeric_direct_chat_name(self):
        chat_id = "447700900123@c.us"
        self.archive.upsert_chat({"id": chat_id, "name": "+447700900123", "archived": False})
        self.archive.replace_inventory_metrics({chat_id: (1, 1_700_000_000, 1_700_000_000)})
        self.archive.confirm_selection()
        self.assertTrue(self.archive.ingest_message({**message(), "chatId": chat_id, "from": chat_id}))

        chat = self.archive.get_chat_policy(chat_id)
        listed, _ = self.archive.list_chat_policies(limit=10, offset=0)

        self.assertEqual(chat["name"], "Alex")
        self.assertEqual(listed[0]["name"], "Alex")
