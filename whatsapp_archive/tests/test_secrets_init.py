...
class SecretInitialisationTests(unittest.TestCase):
    def test_bootstrap_plaintext_is_not_recreated_after_scoped_key_exists(self):
        with tempfile.TemporaryDirectory() as directory:
            original_data, original_secrets = secrets_init.DATA, secrets_init.SECRETS
            try:
                root = Path(directory)
                secrets_init.DATA = root
                secrets_init.SECRETS = root / "secrets"
                (root / "options.json").write_text(
                    json.dumps({"session_name": "atlas_1", "reconcile_seconds": 90}),
                    encoding="utf-8",
                )
                secrets_init.main()
                ...
                secrets_init.DATA, secrets_init.SECRETS = original_data, original_secrets
