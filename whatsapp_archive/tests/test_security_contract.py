...
class SecurityContractTests(unittest.TestCase):
    def test_waha_is_loopback_only_and_not_published(self):
        ...
        self.assertIn("8097/tcp: null", config)
    ...
    def test_home_assistant_privileges_are_disabled(self):
        config = (ADDON / "config.yaml").read_text(encoding="utf-8")
        for setting in ("homeassistant_api: false", "hassio_api: false", "host_network: false"):
    ...
    def test_waha_message_staging_is_excluded_from_cold_backups(self):
        ...
        self.assertIn('"waha/noweb/*/store.sqlite3*"', config)
        self.assertNotIn('"waha/noweb/waha.sqlite3"', config)
        pattern = "data/waha/noweb/*/store.sqlite3*"
        self.assertTrue(
            fnmatch.fnmatchcase("data/waha/noweb/life-atlas/store.sqlite3", pattern)
        )
        self.assertTrue(
            fnmatch.fnmatchcase("data/waha/noweb/life-atlas/store.sqlite3-wal", pattern)
        )
        self.assertFalse(fnmatch.fnmatchcase("data/waha/noweb/waha.sqlite3", pattern))
        self.assertFalse(
            fnmatch.fnmatchcase(
                "data/waha/noweb/life-atlas/.waha.session.config.json", pattern
            )
        )
