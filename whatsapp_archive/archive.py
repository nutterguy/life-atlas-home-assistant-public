...
class Archive:
    ...
    @staticmethod
    def _identity_from_chat_id(chat_id: str) -> str | None:
    ...
    def backfill_display_names(self) -> int:
        """Name a chat from its own identifier when nothing else ever will.

        WhatsApp has migrated direct chats to @lid and @s.whatsapp.net
        addressing, so older @c.us rows are no longer listed by WAHA: they never
        pass through a sync again and so never acquire a name. A phone number is
        a poor label, but it is an identity, and it is already in the row.
        """
        with self.connect() as con:
            rows = con.execute(
                "SELECT chat_id FROM chats WHERE name IS NULL OR name=''"
            ).fetchall()
            named = 0
            for row in rows:
                label = self._identity_from_chat_id(str(row["chat_id"]))
                if not label:
                    continue
                con.execute(
                    "UPDATE chats SET name=? WHERE chat_id=? AND (name IS NULL OR name='')",
                    (label, row["chat_id"]),
                )
                named += 1
        return named
    ...
    def get_chat_policy(self, chat_id: str) -> dict[str, Any] | None:
        ...
        display_name = self._inventory_name_sql("c")
        with self.connect() as con:
            row = con.execute(
                f"""SELECT c.*, {display_name} display_name,
                {expression} effective_selected,
    ...
    def list_chat_policies(
        ...
        with self.connect() as con:
            ...
            rows = con.execute(
                ...
                (SELECT COUNT(*) FROM messages m WHERE m.chat_id=c.chat_id) durable_message_count
                FROM chats c
                ORDER BY c.archived ASC,COALESCE(display_name,c.chat_id) COLLATE NOCASE,c.chat_id
                LIMIT ? OFFSET ?""",
                (limit, offset),
            ).fetchall()
    ...
    def _chat_policy_item(self, row: Mapping[str, Any]) -> dict[str, Any]:
        ...
        return {
            "chat_id": row["chat_id"],
            "name": row["display_name"] if "display_name" in row.keys() else row["name"],
    ...
    @staticmethod
    def _inventory_name_sql(alias: str) -> str:
        """Prefer an archived sender label when WAHA only supplied the chat number."""
        local_id = f"substr({alias}.chat_id,1,instr({alias}.chat_id,'@')-1)"
        numeric_name = f"replace(trim(COALESCE({alias}.name,'')),'+','')={local_id}"
        sender_label = f"""(SELECT NULLIF(trim(m.sender_label),'') FROM messages m
            WHERE m.chat_id={alias}.chat_id AND m.from_me=0
              AND NULLIF(trim(m.sender_label),'') IS NOT NULL
            ORDER BY m.timestamp DESC,m.source_id DESC LIMIT 1)"""
        return f"""CASE WHEN {alias}.chat_type='direct' AND
            (NULLIF(trim({alias}.name),'') IS NULL OR {numeric_name})
            THEN COALESCE({sender_label},{alias}.name)
            ELSE NULLIF(trim({alias}.name),'') END"""
    ...
    def upsert_message(
        ...
        with self.connect() as con:
            if item["chat_id"]:
                ...
                if (
                    not chat_name
                    and not item["from_me"]
                    and item["sender_label"]
                    and "@g.us" not in item["chat_id"]
                ):
                    chat_name = item["sender_label"][:512]
                    con.execute(
                        "UPDATE chats SET name=? WHERE chat_id=? AND (name IS NULL OR name='')",
                        (chat_name, item["chat_id"]),
                    )
                    con.execute(
                        "UPDATE message_fts SET chat_name=? WHERE source_id IN "
                        "(SELECT source_id FROM messages WHERE chat_id=?)",
                        (chat_name, item["chat_id"]),
                    )
    ...
    def _chat_name_sql() -> str:
        return """COALESCE(NULLIF(c.name,''),(
        SELECT NULLIF(peer.sender_label,'') FROM messages peer
        WHERE peer.chat_id=m.chat_id AND peer.from_me=0
          AND peer.sender_label IS NOT NULL AND peer.sender_label!=''
        ORDER BY COALESCE(peer.timestamp,-1) DESC,peer.source_id DESC LIMIT 1
        ))"""
    ...
    def search(self, query: str, *, limit: int, offset: int) -> list[dict[str, Any]]:
        ...
        with self.connect() as con:
            ...
            if backend == "fts5":
                ...
                rows = con.execute(
                    f"""SELECT m.*,{self._chat_name_sql()} chat_name FROM message_fts f
                ...
                rows = con.execute(
                    f"""SELECT m.*,{self._chat_name_sql()} chat_name FROM messages m
                    LEFT JOIN chats c ON c.chat_id=m.chat_id
    ...
    def chat_messages(self, chat_id: str, *, limit: int, offset: int) -> dict[str, Any]:
        """Return a read-only page from an included chat's durable archive."""
        if not chat_id or len(chat_id) > 512:
            raise ValueError("chat_id is invalid")
        if limit < 1 or limit > 100 or offset < 0:
            raise ValueError("limit must be 1–100 and offset must not be negative")
        policy = self.get_chat_policy(chat_id)
        if policy is None:
            raise ValueError("chat was not found")
        if not policy["effective_selected"]:
            raise ValueError("excluded chats cannot be previewed")
        with self.connect() as con:
            rows = con.execute(
                f"""SELECT m.*,{self._chat_name_sql()} chat_name FROM messages m
                LEFT JOIN chats c ON c.chat_id=m.chat_id WHERE m.chat_id=?
                ORDER BY COALESCE(m.timestamp,-1) DESC,m.source_id DESC LIMIT ? OFFSET ?""",
                (chat_id, limit, offset),
            ).fetchall()
        # The query pages newest-first; messages read naturally oldest-to-newest.
        items = [self._source_item(row) for row in reversed(rows)]
        next_offset = offset + len(rows)
        return {
            "chat": policy,
            "items": items,
            "next_offset": next_offset if next_offset < policy["durable_message_count"] else None,
        }

    def context(self, source_id: str, *, before: int, after: int) -> dict[str, Any] | None:
        if before < 0 or after < 0 or before > 20 or after > 20:
            raise ValueError("before and after must be between 0 and 20")
        with self.connect() as con:
            focus = con.execute(
                "SELECT chat_id,COALESCE(timestamp,-1) sort_timestamp FROM messages WHERE source_id=?",
                (source_id,),
            ).fetchone()
            if focus is None or not focus["chat_id"]:
                return None
            common = f"""SELECT m.*,{self._chat_name_sql()} chat_name FROM messages m
            LEFT JOIN chats c ON c.chat_id=m.chat_id"""
            earlier = con.execute(
                common + """ WHERE m.chat_id=? AND
                (COALESCE(m.timestamp,-1)<? OR
                 (COALESCE(m.timestamp,-1)=? AND m.source_id<?))
                ORDER BY COALESCE(m.timestamp,-1) DESC,m.source_id DESC LIMIT ?""",
                (focus["chat_id"], focus["sort_timestamp"], focus["sort_timestamp"], source_id, before),
            ).fetchall()
            current = con.execute(
                common + " WHERE m.source_id=?", (source_id,)
            ).fetchone()
            later = con.execute(
                common + """ WHERE m.chat_id=? AND
                (COALESCE(m.timestamp,-1)>? OR
                 (COALESCE(m.timestamp,-1)=? AND m.source_id>?))
                ORDER BY COALESCE(m.timestamp,-1),m.source_id LIMIT ?""",
                (focus["chat_id"], focus["sort_timestamp"], focus["sort_timestamp"], source_id, after),
            ).fetchall()
        rows = [*reversed(earlier), current, *later]
        items = [self._source_item(row) for row in rows if row is not None]
        return {
            "focus_source_id": source_id,
            "conversation_id": focus["chat_id"],
            "conversation_name": items[0]["metadata"]["conversation_name"] if items else None,
            "items": items,
        }
